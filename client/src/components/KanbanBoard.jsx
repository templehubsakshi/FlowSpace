...
export default function KanbanBoard({
  ...
  return {
  todo: f(tasks.todo || []),
  "in-progress": f(tasks["in-progress"] || []),
  done: f(tasks.done || []),
};
  ...
  const donutData = useMemo(() => [
    ...
    { name: 'In Progress',value: statistics.tasksByStatus?.["in-progress"] || 0, color: T.indigo2 },
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', width: '100%' }}>
            {isLoading ? (
              ...
              <>
                ...
                <KanbanColumn status="in-progress" title="In Progress" tasks={filteredTasks["in-progress"]} onTaskClick={handleTaskClick} onAddTask={() => handleAddTask('in-progress')} onDeleteTask={handleDeleteTask} workspaceId={workspaceId} />
      ...
      <aside style={{
        ...
        <StatCard label="In Progress" value={statistics.tasksByStatus?.["in-progress"] || 0} color={T.indigo2} iconBg="rgba(99,102,241,0.13)"  Icon={Clock} />
