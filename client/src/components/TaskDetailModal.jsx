...
import MentionDropdown from './MentionDropdown'; // ✅ IMPORT
...
export default function TaskDetailModal({ task, onClose }) {
  ...
  const [mentionedUsers, setMentionedUsers] = useState([]); // Array of {id, name}
  ...
  // ✅ HANDLE COMMENT INPUT CHANGE (DETECT @)
  ...
  const handleMentionSelect = (member) => {
    ...
    const newText = `${beforeAt}@${member.name} ${textAfterCursor}`;
  ...
  const handleAddComment = async (e) => {
    ...
    const result = await dispatch(addComment({
      taskId: task._id,
      text: commentText,
      mentions: mentionIds  // ✅ SEND MENTIONS!
    }));
  ...
  const workspaceMembers = currentWorkspace?.members.map(m => ({
    _id: m.user._id,
    name: m.user.name
  })) || [];
              ...
              {mentionedUsers.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  {mentionedUsers.map(u => (
                    <span key={u._id} className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                      @{u.name}
                    </span>
                  ))}
                </div>
