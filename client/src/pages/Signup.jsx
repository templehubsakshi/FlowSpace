...
export default function Signup() {
  ...
  const handleSubmit = async e => {
    ...
    if (result.type === 'auth/signup/fulfilled') toast.success('Account created! 🎉');
  };
      ...
      <div onMouseMove={handleMouseMove} style={{
        ...
        <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
          <div className="aurora-a" style={{
            ...
            background: 'conic-gradient(from 200deg at 38% 55%, rgba(168,85,247,.32) 0deg, rgba(99,102,241,.24) 70deg, rgba(236,72,153,.12) 130deg, transparent 200deg)',
            filter: 'blur(60px)', borderRadius: '50%',
          }}/>
          <div className="aurora-b" style={{
            position: 'absolute', bottom: '-30%', right: '-20%', width: '100%', height: '100%',
            background: 'conic-gradient(from 20deg at 62% 45%, rgba(99,102,241,.26) 0deg, rgba(6,182,212,.15) 80deg, rgba(168,85,247,.2) 150deg, transparent 220deg)',
            filter: 'blur(65px)', borderRadius: '50%',
          }}/>
          <div className="aurora-c" style={{
            position: 'absolute', top: '-5%', right: '0', width: '55%', height: '65%',
            background: 'radial-gradient(ellipse at 70% 30%, rgba(168,85,247,.16) 0%, rgba(99,102,241,.07) 50%, transparent 70%)',
            filter: 'blur(45px)',
          }}/>
          <div style={{
            position: 'absolute', inset: 0, pointerEvents: 'none',
            background: `radial-gradient(ellipse 40% 40% at ${spotX}% ${spotY}%, rgba(168,85,247,.07) 0%, transparent 70%)`,
        ...
        <div className="auth-left-panel left-in" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 56 }}>
            <div style={{
              ...
              boxShadow: '0 0 32px rgba(168,85,247,.9),0 0 0 1px rgba(255,255,255,.16) inset',
            }}>
              <Zap size={21} color="white" strokeWidth={2.5}/>
            </div>
            <span style={{
              fontWeight: 800, fontSize: 20, letterSpacing: '-.04em',
              background: 'linear-gradient(135deg,#f5d0fe,#e0e7ff)',
          ...
          <div style={{ marginBottom: 20 }}>
            <h2 style={{
              ...
              fontFamily: "'Plus Jakarta Sans',sans-serif",
              color: '#f8fafc', textShadow: '0 0 80px rgba(168,85,247,.25)',
            }}>Start building</h2>
        ...
        <div className="auth-right-panel" style={{
          ...
          <div className="card-in" ref={cardRef} style={{
            ...
            <div style={{
              ...
              background: 'linear-gradient(150deg,rgba(255,255,255,.075) 0%,rgba(255,255,255,.022) 55%,rgba(168,85,247,.03) 100%)',
              ...
              <div style={{
                position: 'absolute', inset: 0, pointerEvents: 'none',
                background: `radial-gradient(circle 200px at ${mouse.x * 100}% ${mouse.y * 100}%, rgba(168,85,247,.08) 0%, transparent 60%)`,
              ...
              <div style={{ padding: '36px 34px 32px', position: 'relative' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                  ...
                  <div className="field-in" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <Lock size={14} style={ist('confirm')}/>
                      <input
                        type="password" value={confirm}
                        onChange={e => setConfirm(e.target.value)}
                        onFocus={() => setFocus('confirm')} onBlur={() => setFocus('')}
                        placeholder="••••••••" disabled={isLoading}
                        style={{ ...inp('confirm'), opacity: isLoading ? .5 : 1 }}
                      />
                    </div>
                  </div>
                  ...
                  <button type="submit" disabled={isLoading} className="cta-btn" style={{
                    ...
                    background: isLoading ? 'rgba(168,85,247,.3)' : 'linear-gradient(135deg,#7c3aed 0%,#a855f7 100%)',
                    border: 'none', cursor: isLoading ? 'not-allowed' : 'pointer',
                    color: 'white', fontSize: 14.5, fontWeight: 700,
                    fontFamily: "'Plus Jakarta Sans',sans-serif", letterSpacing: '-.02em',
                    boxShadow: '0 6px 30px rgba(168,85,247,.55),0 0 0 1px rgba(255,255,255,.1) inset',
                  }}>
                    {isLoading
                      ? <><div className="spin-el" style={{ width: 16, height: 16, borderRadius: '50%', border: '2.5px solid rgba(255,255,255,.2)', borderTopColor: 'white', flexShrink: 0 }}/>Creating…</>
                      : <><Sparkles size={16} strokeWidth={2}/>Create Account</>
                    }
                  </button>
                </form>
